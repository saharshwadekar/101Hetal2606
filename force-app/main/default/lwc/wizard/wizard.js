...
export default class Wizard extends LightningElement {
    ...
    @api header = '';
    @api isNextStepVisible;
    @api isPreviousStepVisible;
    @api hideProgress = false;
    customNextStepLabel;
    ...
    get getNextLabel() {
        return this.customNextStepLabel ? this.customNextStepLabel : this.isLast() ? this.finishLabel:this.nextLabel;
    }

    get getPreviousLabel() {
        return this.isFirst()? this.cancelLabel:this.previousLabel;
    }
    ...
    setActiveStep(stepName) {
        ...
        Object.values(self.steps).forEach(function (step) {
            ...
            if(step.name === self._currentStep){
                self.isNextStepVisible = step.methods.getIsNextStepVisible();
                self.isPreviousStepVisible = step.methods.getIsPreviousStepVisible();
                self.customNextStepLabel = step.methods.getNextStepLabel();
            }
        });
    ...
    */
    ...
    isFirst(){
        let currentStep = this.steps[this._currentStep];
        return this.flow.indexOf(currentStep) == 0;
    }

    isLast(){
        let currentStep = this.steps[this._currentStep];
        return this.flow.indexOf(currentStep) == this.flow.length-1;
    }
    
    handlePrevious() {
        this.moveStep('previous');
    }
